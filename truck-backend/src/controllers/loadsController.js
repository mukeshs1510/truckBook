...
class LoadController {
  async addLoad(req, res) {
    try {
      ...
      if (!weight) {
        return res.status(400).json({ message: 'Please, enter load\'s total weight' });
      }
      const { loadType } = req.body;
      if (!loadType) {
        return res.status(400).json({ message: 'Please, enter load\'s type' });
      }
      const { date } = req.body;
      if (!date) {
        return res.status(400).json({ message: 'Please, enter load\'s shipment date' });
      }
      ...
      const load = new Load({
        ...
        delieverTill: req.body.delieverTill
      });
      await load.save()
  ...
  async getLoadsTruck(req, res) {
    // console.log('res.user',req.user)
    try {
      // const { userId } = req.user;
      const loadList = await LoadService.getLoadsForTruck();
      return res.json({ loads: loadList });
    } catch (err) {
      return res.status(400).json({ message: err.message });
    }
  ...
  async getTruckAssigned(req, res) {
    // console.log('res.user',req.user)
    try {
      console.log(req.path.split('/')[2])
      const assignedTo = req.path.split('/')[2];
      console.log('assignedTo', assignedTo)
      const loadList = await LoadService.getAssignedLoads(assignedTo);
      return res.json({ loads: loadList });
    } catch (err) {
      return res.status(400).json({ message: err.message });
    }
