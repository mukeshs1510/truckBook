...
const getLoadsForTruck = async () => {
  try {
    const loadList = await Load.find({ status: 'NEW' }).select('-__v');
    return loadList;
  } catch (err) {
    throw Error('Error while searching for user\'s loads');
  }
...
const getAssignedLoads = async (assgnedTo) => {
  try {
    const loadList = await Load.find({ status: 'ASSIGNED', assigned_to: assgnedTo }).select('-__v');
    return loadList;
  } catch (err) {
    throw Error('Error while searching for user\'s loads');
  }
